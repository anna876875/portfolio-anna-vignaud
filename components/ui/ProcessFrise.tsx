...
export function ProcessFrise({
  ...
  /* ── Layout mobile : accordéon vertical ── */
  if (isMobile) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        {phases.map((p, i) => {
          ...
          return (
            <div key={p.id} style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
              {/* Étape */}
              <motion.button
              ...
              {/* Panel inline sous le point actif */}
              <AnimatePresence>
                {on && (
                  <motion.div
                    key={p.id + "-panel"}
                    ...
                    transition={{ duration: 0.25, ease: SP }}
                    style={{ width: "100%", overflow: "hidden" }}>
                    <div style={{ paddingTop: 12, paddingBottom: 8, width: "100%" }}>
                      <div className="mat-regular" style={{
                        borderRadius: "var(--r-xl) var(--r-xl) 0 0",
                        border: "1px solid rgba(255,255,255,0.45)", borderBottom: "none",
                        padding: "14px 16px", display: "flex", alignItems: "flex-start", gap: 10, flexWrap: "wrap",
                      }}>
                        ...
                        {p.livrable && (
                          <span className="hig-footnote" style={{ color: "var(--color-label-3)", width: "100%", paddingLeft: 18 }}>
                            {p.livrable}
                          </span>
                        )}
                      </div>
                      <div className="mat-regular" style={{
                        ...
                        borderRadius: "0 0 var(--r-xl) var(--r-xl)", padding: "var(--sp-5) 16px",
                      }}>
                        {renderPanel(p.id)}
                      </div>
                    </div>
  ...
  /* ── Layout desktop : tabs + panel sous le stepper ── */
  return (
    <div>
      {/* Stepper */}
